...
def main():
    ...
    fs = 100
    winsize = 20
    div = 6 # divide 1 sample as div samples

    ## ---------------------------------------------------------------------------------------------------------------------
    ## Save dir
    now = datetime.utcnow().strftime("%Y%m%d%H%M%S")
    board_logdir = "C:\\Users\\SNUGL\\tensorboard2\\"
    ckpt_logdir = "D:\dev\jejucamp-seoyeon\classification\\model_ckpt2\\"
    ...
    if check == 0 :
    # training data
        train_parser = data_loader3.parse_cambridge() # class
    ...
    # read the data and concatinate
        train_data, train_label, train_height, train_width = train_parser.loading_data(train_path_list, div, 'train', fs, winsize)
        np.save('train_data_3', train_data)
        np.save('train_label_3', train_label)
    # test data
        test_parser = data_loader3.parse_cambridge()
    # name list, label list, path list parser
        test_name_list, test_label_list, test_path_list = test_parser.label_extract('test')
    # read the data and concatinate
        test_data, test_label, test_height, test_width  = test_parser.loading_data(test_path_list, div, 'test', fs, winsize)
        np.save('test_data_3', test_data)
        np.save('test_label_3', test_label)
        check = 2

        # train, test height needed to same = div need to be same
    if check == 1 :
        train_data = np.load('train_data_3.npy')
        train_label = np.load('train_label_3.npy')
        test_data = np.load('test_data_3.npy')
        test_label = np.load('test_label_3.npy')
        check = 2
        train_height = train_data.shape[1]
        train_width = train_data.shape[2]
    ## ---------------------------------------------------------------------------------------------------------------------
    ## check point loading
    ...
    with tf.Session(graph=jeju_graph) as sess:
        ...
        for e in range(epochs) :
            ...
            for i in range(total_batch) :
                random_idex = random.randrange(0, train_len - batch_size)
                batch_validation_data = train_data[random_idex:random_idex + batch_size, ...]
                batch_validation_label = train_label[random_idex:random_idex + batch_size, ...]
